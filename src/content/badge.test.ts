...
describe('Badge UI', () => {
  describe('createBadge', () => {
    it('should create badge element with count for expanded epic', () => {
      ...
      expect(badge.title).toContain('5 unique assignees');
    });

    it('should create badge with placeholder for collapsed epic', () => {
      const badge = createBadge(0, false);

      expect(badge.textContent).toBe('? 👥');
      expect(badge.title).toContain('Expand epic to see assignee count');
    });
  ...
  describe('injectBadge', () => {
    ...
    it('should not inject duplicate badge', () => {
      ...
      const badges = epicRow.querySelectorAll('.jira-plans-headcount-badge');
      expect(badges.length).toBe(1);
      expect(badges[0].textContent).toBe('3 👥'); // Original badge unchanged
    });

    it('should return false if epic title not found', () => {
      document.body.innerHTML = `
        <div data-issue="18794394" data-name="scope-issue-18794394">
          <a class="_3HCO4" href="/browse/GS-22732">GS-22732</a>
        </div>
  ...
  describe('updateBadge', () => {
    it('should update existing badge content', () => {
      ...
      expect(badge?.title).toContain('7 unique assignees');
    });
