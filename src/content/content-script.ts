/**
 * Content Script for Jira Plans Headcount Extension
 *
...
import { injectSprintBadges, injectTimelineBadge, updateTimelineBadge, injectBadge, updateBadge, countBadges, clearAllBadges, clearTimelineBadges, BADGE_CLASS, TIMELINE_BADGE_CLASS } from './badge';
...
// Key: epicKey (e.g., "GS-12345"), Value: {totalCount, sprintCounts, timestamp, unscheduledStories, assigneeNames}
...
let statistics: ExtensionStatistics = { ...INITIAL_STATISTICS };
...
function updateTimelineBadgesWithSprints(issueId: string, sprintCounts: Map<string, number>, totalCount: number, unscheduledStories?: string[], sprintAssignees?: Map<string, string[]>): void {
  ...
  const sprintBadgeData: Array<{ sprintName: string; count: number; positionPercent: number }> = [];
...
export async function initialize() {
  ...
  const observer = setupObserver(currentSettings.performance.debounceDelayMs);
